...
public class GamePlayZombieMovement : MonoBehaviour, IMoveable
{
    ...
    private bool hasAdjustedHeight = false; // Flag to check if height has been adjusted
        ...
        if (isFollowingInput)
        {
            ...
            if (ZombieSpeedManager.Instance != null && moveVector != null)
            {
                ...
                if (moveVector.magnitude > 0)
                {
                    ...
                    // Adjust height if not done yet
                    AdjustHeightIfMoving();
                }
                else
                {
                    // Play the idle animation
    ...
    private void AdjustHeightIfMoving()
    {
        if (!hasAdjustedHeight)
        {
            ...
            // Set the flag to true so it doesn't adjust height again
