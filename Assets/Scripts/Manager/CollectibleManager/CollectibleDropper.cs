...
public class CollectibleDropper : MonoBehaviour
{
    public string[] collectibleTags; // Tags to identify collectibles in the pool
    public float[] dropProbabilities; // Array to hold drop probabilities for each collectible
    public float dropChance = 0.1f; // Chance for any collectible to drop (e.g., 0.5 for 50%)
    public LayerMask groundLayer; // Layer mask to define what layers are considered ground
    ...
    private void Start()
    {
        if (collectibleTags.Length != dropProbabilities.Length)
        {
            Debug.LogError("Collectibles tags and drop probabilities arrays must have the same length.");
            return;
        }
    }
    ...
    public void TryDropCollectible(Vector3 position)
    {
        ...
        if (Random.Range(0f, 1f) <= dropChance)
        {
            ...
            for (int i = 0; i < collectibleTags.Length; i++)
            {
                ...
                if (randomValue <= cumulativeProbability)
                {
                    DropCollectible(collectibleTags[i], position);
                    break;
                }
            }
        }
    ...
    private void DropCollectible(string tag, Vector3 position)
    {
        Vector3 groundPosition = FindGroundPosition(position);
        GameObject collectible = ObjectPooler.Instance.SpawnFromPool(tag, groundPosition, Quaternion.identity);
        if (collectible == null)
        {
            Debug.LogError("Failed to spawn collectible with tag: " + tag);
        }
    }
