...
public class ZombieDetectionForAttackers : MonoBehaviour
{
    ...
    // Pre-allocated array for detection
    private Collider[] detectedColliders;
    [SerializeField] private int maxDetectedZombies = 400; // Set this value based on your expected maximum number of zombies
    ...
    void Start()
    {
        ...
        // Initialize the pre-allocated array
        detectedColliders = new Collider[maxDetectedZombies];
    }
    ...
    void DetectZombiesInRange()
    {
        // Use Physics.OverlapSphereNonAlloc to detect all colliders within the detection range
        int detectedCount = Physics.OverlapSphereNonAlloc(transform.position, detectionRange, detectedColliders, zombieLayer);
