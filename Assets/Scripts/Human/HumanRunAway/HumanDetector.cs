...
public class HumanDetector : MonoBehaviour
{
    ...
    private Collider[] collidersBuffer = new Collider[10]; // Buffer for storing colliders, adjust the size as needed
    ...
    void Update()
    {
        ...
        // Find all human objects within the detection range using OverlapSphereNonAlloc
        int colliderCount = Physics.OverlapSphereNonAlloc(transform.position, detectionRange, collidersBuffer);
        ...
        if (isZombieInRange)
        {
            foreach (Transform human in detectedHumans)
            {
                HumanMovement humanMovement = human.GetComponent<HumanMovement>();
                if (humanMovement != null && humanMovement.IsMainHuman(centralHuman))
                {
                    humanMovement.MoveToEscapePoint(); // Move human to escape point
                }
            }
        }
    }
